...
evaluation = dict(
    interval=1, metric='PCK', key_indicator='PCK', gpu_collect=True)
optimizer = dict(
    type='Adam',
    lr=5e-4,
)
...
model = dict(
    ...
    pretrained='work_dirs/finetune_split3/latest.pth',
    backbone=dict(type='ResNet', depth=50),
    deconv=dict(type='CustomDeconv', in_channels=2048),
    keypoint_head=dict(
        type='ESCAPEHead',
        matching_network=dict(
            type='SuperGlue',
            backbone=dict(type='ResNet', depth=50),
            deconv=dict(type='CustomDeconv', in_channels=2048),
            ...
            mlp_dim=2048),
        superkeypoints=dict(
            checkpoint='work_dirs/finetune_split3/latest.pth',
            mname='keypoint_head.keypoints'),
...
data = dict(
    ...
    train=dict(
        ...
        superkeypoints='work_dirs/baseline_split3/superkeypoints.pth',
        data_cfg=data_cfg,
