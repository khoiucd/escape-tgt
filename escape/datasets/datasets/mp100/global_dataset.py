...
from .global_base_dataset import GlobalBaseDataset


@DATASETS.register_module()
class GlobalDataset(GlobalBaseDataset):
    ...
    def __init__(self,
        ...
        self.kpt_num = {}
        for _ in self.db:
            if _['category_id'] not in self.kpt_num:
                self.kpt_num[_['category_id']] = _['cat_kpt_num']

        self.keypoint_num = sum([self.kpt_num[_] for _ in self.kpt_num])

        self.keypoint_index = {}
        self.init_keypoint_index()

        self.keypoint_index_onehot = {}

        if superkeypoints is not None:
            self.superkeypoints = torch.load(superkeypoints)
            self.get_superkeypoint_index_mapping()
        else:
            self.get_keypoint_index_mapping()

    def init_keypoint_index(self):
        idx = 0
        for c in self.kpt_num:
            self.keypoint_index[c] = [idx + _ for _ in range(self.kpt_num[c])]
            idx += self.kpt_num[c]

    def get_keypoint_index_mapping(self):
        for c in self.kpt_num:
            self.keypoint_index_onehot[c] = torch.zeros(
                self.max_kpt_num, self.keypoint_num)
            for i, j in enumerate(self.keypoint_index[c]):
                self.keypoint_index_onehot[c][i, j] = 1

    def get_superkeypoint_index_mapping(self):
        _keypoint_to_superkeypoint = self.superkeypoints[
            '_keypoint_to_superkeypoint']
        for c in self.kpt_num:
            self.keypoint_index_onehot[c] = torch.zeros(
                self.max_kpt_num, self.superkeypoints['superkeypoint_num'])
            for i, j in enumerate(self.keypoint_index[c]):
                k = _keypoint_to_superkeypoint[j]
                self.keypoint_index_onehot[c][i, k] = 1
    ...
    def _load_coco_keypoint_annotation_kernel(self, img_id):
        ...
        for obj in objs:
            ...
            if category_id not in self.zerind:
                self.zerind[category_id] = self.z
                self.z += 1
            ...
            rec.append({
                ...
                'zerind_category_id': self.zerind[category_id],
    ...
    def evaluate(self,
        ...
        kpts = []
        kpts_features = [[] for k in range(self.keypoint_num)]
            ...
            for i in range(batch_size):
                ...
                for j, k in enumerate(keypoint_index[i]):
                    if target_weight[i, j].item() > 0:
                        kpts_features[k].append(keypoint_features[i, j])
